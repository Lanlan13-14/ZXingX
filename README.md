...
- ZXingX 自绘边缘返回：页面跟手缩放、圆角化、取消回弹 / 松手连续退出；顶部返回按钮使用独立的纯水平向右滑出
...
4. 用 35mm 等效焦距计算相对视角，双指跨焦段时先尝试 `setPhysicalCameraId()`，失败再尝试独立 Camera ID，最后回退逻辑相机普通变焦。
5. 需要重建会话时冻结当前预览帧；新镜头流进入 `STREAMING` 后 160ms 交叉淡出，避免直接露出黑帧。
